...
function RentalYieldAnalysisChart() {
  ...
  const getInvestmentData = () => {
    return yieldData.map(item => {
      const downPaymentRatio = 0.2; // 20% down payment
      ...
      const monthlyMortgagePayment =
        ...
        12 /
        (1 - Math.pow(1 + mortgageRate / 12, -300)); // 25 year amortization

      const monthlyRent = item.avgRent;
      const monthlyExpenses = monthlyRent * 0.3; // 30% of rent for expenses (property tax, insurance, maintenance, vacancy)
      ...
      return {
        ...
        rentToPrice,
        breakEvenRatio:
          (monthlyMortgagePayment + monthlyExpenses) / monthlyRent,
      };
    });
  };
    ...
    switch (viewMode) {
      ...
      case 'comparison':
        return regions.map(region => {
          ...
          const avgGrossYield =
            regionData.reduce((sum, item) => sum + item.grossYield, 0) /
            regionData.length;
          const avgNetYield =
            regionData.reduce((sum, item) => sum + item.netYield, 0) /
            regionData.length;
          const avgPrice =
            regionData.reduce((sum, item) => sum + item.avgPrice, 0) /
            regionData.length;
          const avgRent =
            regionData.reduce((sum, item) => sum + item.avgRent, 0) /
            regionData.length;
  ...
  const avgGrossYield =
    investmentData.reduce((sum, item) => sum + item.grossYield, 0) /
    investmentData.length;
  const avgNetYield =
    investmentData.reduce((sum, item) => sum + item.netYield, 0) /
    investmentData.length;
  const avgCashFlow =
    investmentData.reduce((sum, item) => sum + item.monthlyCashFlow, 0) /
    investmentData.length;
            ...
            {viewMode === 'cashflow' && (
              <>
                ...
                <p className="text-sm">
                  ...
                  <span className="font-medium ml-2 text-red-600">
                    -${data.monthlyMortgage?.toLocaleString()}
                  </span>
                </p>
                <p className="text-sm">
                  <span className="text-gray-600">Net Cash Flow:</span>
                  <span
                    className={`font-medium ml-2 ${data.monthlyCashFlow >= 0 ? 'text-green-600' : 'text-red-600'}`}
                  >
            ...
            {viewMode === 'comparison' && (
              <>
                ...
                <p className="text-sm">
                  ...
                  <span className="font-medium ml-2">
                    ${(data.avgPrice * 1000).toLocaleString()}
                  </span>
                </p>
                <p className="text-sm">
      ...
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center mb-6">
        <div>
          ...
          <p className="text-sm text-gray-600">
            Comprehensive investment analysis including yields, cash flow, and
            ROI metrics
          </p>
        </div>
            ...
            {viewMode === 'roi' ? (
              ...
              >
                ...
                <XAxis
                  ...
                  label={{
                    value: 'Property Price (thousands)',
              ...
              >
                ...
                <YAxis
                  tickFormatter={
                    viewMode === 'cashflow' ? formatCurrency : formatPercent
                  }
                  tick={{ fontSize: 12 }}
                  tickLine={false}
                  axisLine={{ stroke: '#e5e7eb' }}
        ...
        <div className="space-y-4">
          ...
          <div className="p-3 bg-purple-50 rounded-lg border-l-4 border-purple-500">
            ...
            <p className="text-xs text-purple-600">
              {bestYield.grossYield}% gross yield
            </p>
          </div>
        </div>
      </div>
              ...
              >
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex justify-between items-center">
                    ...
                    <span
                      className={`font-medium ${isPositiveCashFlow ? 'text-green-600' : 'text-red-600'}`}
                    >
                  ...
                  <div className="flex justify-between items-center">
                    ...
                    <span className="font-medium text-purple-600">
                      {item.cashOnCashReturn.toFixed(1)}%
                    </span>
                  </div>
                  <div className="flex justify-between items-center">
      ...
      <div className="mt-6 grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="bg-blue-50 rounded-lg p-4">
          ...
          <p className="text-sm text-blue-600">
            {bestYield.type} properties offer the highest gross yields at{' '}
            {bestYield.grossYield}%, making them attractive for income-focused
            investors.
          </p>
        </div>
        ...
        >
          <h5
            className={`font-semibold mb-2 ${
              bestCashFlow.monthlyCashFlow >= 0
                ? 'text-green-800'
                : 'text-red-800'
            }`}
          >
          ...
          >
            {bestCashFlow.type} properties generate $
            {Math.abs(bestCashFlow.monthlyCashFlow).toFixed(0)}/month in{' '}
            {bestCashFlow.monthlyCashFlow >= 0
              ? 'positive cash flow'
              : 'negative cash flow'}
            ,
            {bestCashFlow.monthlyCashFlow >= 0
              ? ' ideal for leveraged investments'
              : ' requiring additional capital injection'}
            .
          </p>
        </div>
        ...
        <div className="bg-purple-50 rounded-lg p-4">
          ...
          <p className="text-sm text-purple-600">
            With average net yields of {avgNetYield.toFixed(1)}%, this market
            offers {avgNetYield > 3 ? 'competitive' : 'moderate'} returns for
            real estate investors seeking rental income diversification.
          </p>
        </div>
      </div>
    </div>
