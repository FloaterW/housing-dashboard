...
function RegionalComparison({ selectedHousingType }) {
  ...
  const metrics = [
    ...
    { id: 'pricePerSqFt', name: 'Price per Sq Ft', icon: '📐', color: '#8B5CF6' },
    { id: 'daysOnMarket', name: 'Days on Market', icon: '📅', color: '#F59E0B' },
    { id: 'absorptionRate', name: 'Absorption Rate', icon: '📊', color: '#10B981' },
  ];
        ...
        <div className="bg-white p-4 rounded-lg shadow-xl border border-gray-200">
          ...
          <div className="space-y-1">
            ...
            <div className={`flex justify-between items-center pt-2 border-t ${
              (difference > 0 && isHigherBetter) || (difference < 0 && !isHigherBetter)
                ? 'text-green-600'
                : 'text-red-600'
            }`}>
              <span className="text-sm font-medium">Difference:</span>
              <span className="font-bold">{difference > 0 ? '+' : ''}{percentDiff}%</span>
            </div>
          </div>
        </div>
      );
  ...
  const getBarColor = (value, ontarioAvg) => {
    ...
    if ((difference > 0 && isHigherBetter) || (difference < 0 && !isHigherBetter)) {
    ...
    <div className="bg-white rounded-xl shadow-lg p-6 transform transition-all duration-300 hover:shadow-xl">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 space-y-4 sm:space-y-0">
        <div>
          ...
          <p className="text-sm text-gray-600 mt-1">Compare {selectedHousingType} across regions</p>
        </div>
        ...
        >
          <defs>
            <linearGradient id="colorGradient" x1="0" y1="0" x2="1" y2="0">
              ...
              <stop offset="95%" stopColor={currentMetric.color} stopOpacity={0.6}/>
            </linearGradient>
          </defs>
          ...
          <Bar 
            dataKey="value" 
            name={`${selectedHousingType} - ${currentMetric.name}`}
            radius={[0, 8, 8, 0]}
          >
      ...
      <div className="mt-6 p-4 bg-gray-50 rounded-lg">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          {data.map((region) => {
            const diff = ((region.value - region.ontarioAverage) / region.ontarioAverage * 100).toFixed(1);
            const isPositive = (diff > 0 && isHigherBetter) || (diff < 0 && !isHigherBetter);
